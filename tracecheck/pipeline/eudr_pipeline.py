"""EUDR analysis pipeline orchestrator.

Runs for a single AnalysisJob:
  1. Load all parcels for the project
  2. For each parcel: fetch Sentinel-2 → detect change → score risk → save result
  3. Update job status throughout
"""

from __future__ import annotations

import json
import logging
from pathlib import Path

from sqlalchemy.ext.asyncio import AsyncSession

from tracecheck.config import settings
from tracecheck.core.change_detector import EUDRChangeDetector
from tracecheck.core.risk_scorer import score_risk
from tracecheck.core.sentinel_fetcher import SentinelFetcher
from tracecheck.db import crud

logger = logging.getLogger(__name__)


async def run_eudr_analysis(job_id: str, db: AsyncSession) -> None:
    """Execute a full EUDR analysis job.

    This function is designed to run as a background task.
    It updates job status in the DB throughout execution.

    Args:
        job_id: UUID of the AnalysisJob to run.
        db: AsyncSession — NOTE: must be a NEW session, not shared with request.
    """
    logger.info("Starting EUDR analysis job: %s", job_id)

    # ── Load job ──────────────────────────────────────────────────────────────
    job = await crud.get_job(db, job_id)
    if not job:
        logger.error("Job not found: %s", job_id)
        return

    await crud.update_job_status(db, job_id, "running")
    await db.commit()

    parcels = await crud.list_parcels(db, job.project_id)
    if not parcels:
        await crud.update_job_status(db, job_id, "done", processed_parcels=0)
        await db.commit()
        logger.info("Job %s: no parcels to process", job_id)
        return

    # ── Get project cutoff date ───────────────────────────────────────────────
    project = await db.get(__import__("tracecheck.db.models", fromlist=["Project"]).Project, job.project_id)
    cutoff_date = getattr(project, "cutoff_date", settings.eudr_cutoff_date) if project else settings.eudr_cutoff_date

    # ── Initialize components ─────────────────────────────────────────────────
    fetcher = SentinelFetcher(data_dir=Path(settings.data_dir) / "sentinel2")
    detector = EUDRChangeDetector(
        ndvi_threshold=settings.ndvi_threshold,
        min_area_ha=settings.min_changed_area_ha,
    )

    processed = 0
    errors = 0

    for parcel in parcels:
        try:
            # Extract bbox from parcel
            bbox = _get_bbox(parcel)

            # Fetch Sentinel-2 scenes
            before_tif, after_tif, before_info, after_info = fetcher.fetch_for_parcel(
                parcel_id=parcel.id,
                bbox=bbox,
                cutoff_date=cutoff_date,
            )

            # Run change detection
            change_result = detector.detect(
                parcel_id=parcel.id,
                before_tif=before_tif,
                after_tif=after_tif,
                geojson_str=parcel.geojson,
                before_scene_date=before_info.acquisition_date,
                after_scene_date=after_info.acquisition_date,
            )

            # Score risk
            risk = score_risk(change_result)

            # Save result
            await crud.save_parcel_result(db, {
                "job_id": job_id,
                "parcel_id": parcel.id,
                "risk_level": risk.risk_level,
                "ndvi_before": change_result.ndvi_before,
                "ndvi_after": change_result.ndvi_after,
                "delta_ndvi": change_result.delta_ndvi,
                "nbr_before": change_result.nbr_before,
                "nbr_after": change_result.nbr_after,
                "delta_nbr": change_result.delta_nbr,
                "changed_area_ha": change_result.changed_area_ha,
                "cloud_fraction": change_result.cloud_fraction,
                "confidence": risk.confidence,
                "flag_reason": risk.flag_reason,
                "before_scene_date": change_result.before_scene_date,
                "after_scene_date": change_result.after_scene_date,
                "data_source": change_result.data_source,
            })
            await db.commit()

            processed += 1
            if processed % 10 == 0:
                await crud.update_job_status(db, job_id, "running", processed_parcels=processed)
                await db.commit()

            logger.debug(
                "Job %s parcel %s → %s (dNDVI=%.3f)",
                job_id, parcel.id[:8], risk.risk_level, change_result.delta_ndvi,
            )

        except Exception as exc:
            errors += 1
            logger.warning("Job %s: parcel %s failed: %s", job_id, parcel.id[:8], exc)
            # Save failed result as REVIEW
            try:
                await crud.save_parcel_result(db, {
                    "job_id": job_id,
                    "parcel_id": parcel.id,
                    "risk_level": "review",
                    "flag_reason": f"processing_error: {str(exc)[:100]}",
                    "confidence": 0.0,
                    "data_source": "Copernicus Sentinel-2",
                })
                await db.commit()
                processed += 1
            except Exception:
                pass

    # ── Finalise ──────────────────────────────────────────────────────────────
    final_status = "done" if errors < len(parcels) else "failed"
    err_msg = f"{errors}/{len(parcels)} parcels failed" if errors > 0 else None
    await crud.update_job_status(
        db, job_id, final_status,
        error_message=err_msg,
        processed_parcels=processed,
    )
    await db.commit()
    logger.info(
        "Job %s complete: %d processed, %d errors, status=%s",
        job_id, processed, errors, final_status,
    )


def _get_bbox(parcel) -> tuple[float, float, float, float]:
    """Extract bounding box from a Parcel ORM object."""
    # Use cached bbox if available
    if all(v is not None for v in [parcel.bbox_minx, parcel.bbox_miny, parcel.bbox_maxx, parcel.bbox_maxy]):
        return (parcel.bbox_minx, parcel.bbox_miny, parcel.bbox_maxx, parcel.bbox_maxy)

    # Fall back to parsing geojson
    geojson = json.loads(parcel.geojson)
    geometry = geojson.get("geometry") or geojson
    geom_type = geometry.get("type", "")
    coords = geometry.get("coordinates", [])

    if geom_type == "Point":
        lon, lat = coords[0], coords[1]
        # 1km buffer for points
        delta = 0.01  # ~1.1km
        return (lon - delta, lat - delta, lon + delta, lat + delta)

    if geom_type == "Polygon":
        flat = [c for ring in coords for c in ring]
        lons = [c[0] for c in flat]
        lats = [c[1] for c in flat]
        # Small buffer
        buf = 0.001
        return (min(lons) - buf, min(lats) - buf, max(lons) + buf, max(lats) + buf)

    raise ValueError(f"Cannot extract bbox from geometry type: {geom_type}")
